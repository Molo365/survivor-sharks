...
export const GetPickEmDashboardStatsResponseItem = zod.object({
  ...
  "sovPrizeWon": zod.number().nullish().describe('Survivor SOV tiebreaker only: this user\'s actual prize won based on their rank and prize structure (scaled to actual entries)'),
...
export const GetPoolResponse = zod.object({
  ...
  "entryFee": zod.number().nullish(),
  "prizePot": zod.number().nullish(),
  "commissionerCut": zod.number().describe('Commissioner percentage retained from the prize pool'),
  "showCommissionerCut": zod.boolean().describe('Whether the commissioner cut is disclosed to pool members'),
  "doubleElimination": zod.boolean().optional(),
  "pickFrequency": zod.enum(['weekly', 'daily']).optional().describe('MLB only: pick frequency for this pool'),
  "isRecurring": zod.boolean().describe('MLB Daily only: false = one-off pool that stops after day 1; true = auto-advances indefinitely'),
...
export const UpdatePoolBody = zod.object({
  ...
  "sandboxMode": zod.boolean().optional().describe('Enable sandbox mode (uses hardcoded schedule + simulated scores). Super-admin only.'),
  "prizeStructure": zod.array(zod.object({
  "place": zod.number().describe('Finishing position (1 = 1st place)'),
  "amount": zod.number().describe('Prize amount in dollars')
})).optional()
...
export const GetPickEmGamesResponse = zod.object({
  ...
  "poolNotStarted": zod.boolean().optional().describe('True when an MLB weekly Pick-Em pool is waiting for its configured initial period'),
  "startsAt": zod.coerce.date().optional().describe('Configured initial period start date when poolNotStarted is true'),
...
export const SubmitPickEmPicksBody = zod.object({
  ...
  "sendConfirmation": zod.boolean().optional().describe('Request one pick confirmation email after the final request of a live NHL weekly submission')
})
 ...
 */
export const GetNdpWeek18GamesParams = zod.object({
  "poolId": zod.coerce.number()
})

export const GetNdpWeek18GamesResponseItem = zod.object({
  "id": zod.string().describe('ESPN game ID (or sandbox game ID in sandbox mode)'),
  "awayTeam": zod.string().describe('Away team display name'),
  "homeTeam": zod.string().describe('Home team display name'),
  "startTime": zod.string().describe('Game start time (ISO 8601)')
})
export const GetNdpWeek18GamesResponse = zod.array(GetNdpWeek18GamesResponseItem)
...
export const GetNflPickEmSeasonLeaderboardResponse = zod.object({
  ...
  "liveGamesInProgress": zod.number().optional(),
  "entries": zod.array(zod.object({
  "rank": zod.number(),
  "userId": zod.number(),
  "username": zod.string(),
  ...
  "liveCorrect": zod.number().optional(),
