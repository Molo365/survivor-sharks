...
const ESPN_TEAMS: Record<Sport, { id: string; name: string; abbreviation: string; location: string; conference?: string; division?: string }[]> = {
  nfl: [
...
function getLogoUrl(sport: Sport, teamId: string): string | null {
  if (sport === "fifa") {
    return `https://flagcdn.com/w80/${teamId.toLowerCase()}.png`;
  }
...
router.get("/:sport/schedule/:week", requireAuth, async (req, res) => {
  ...
  // Attempt to fetch live schedule from ESPN API
  try {
  ...
  for (let i = 0; i < teams.length - 1; i += 2) {
    games.push({
      ...
      sport,
      homeTeam: { id: teams[i].id, name: teams[i].name, abbreviation: teams[i].abbreviation, sport, logoUrl: getLogoUrl(sport, teams[i].id), location: teams[i].location, conference: teams[i].conference ?? null, division: teams[i].division ?? null, flagUrl: null },
      awayTeam: { id: teams[i + 1].id, name: teams[i + 1].name, abbreviation: teams[i + 1].abbreviation, sport, logoUrl: getLogoUrl(sport, teams[i + 1].id), location: teams[i + 1].location, conference: teams[i + 1].conference ?? null, division: teams[i + 1].division ?? null, flagUrl: null },
