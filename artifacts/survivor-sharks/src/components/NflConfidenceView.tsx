    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 rounded-lg border border-purple-500/30 bg-purple-500/5 p-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Your NFL Confidence picks are locked in. Good luck!</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border/40 bg-card/50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-bebas text-2xl text-purple-300">{tiebreakerPassingYards ?? "—"}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground/70 mb-0.5">Combined rushing yards</p>
            <p className="font-bebas text-2xl text-purple-300">{tiebreakerRushingYards ?? "—"}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    return (
      <button
        ...
        className={cn(
          ...
          isPicked ? "bg-purple-500/12" : !isGameLocked ? "hover:bg-muted/20 active:bg-muted/30" : "",
      ...
      >
        ...
        <div className={cn("min-w-0 flex-1", isHome ? "text-right" : "text-left")}>
          ...
          {(isFinal || isLive) && score != null ? (
            <div className={cn("font-bebas text-2xl leading-none", isPicked ? "text-purple-300" : "text-foreground/70")}>
              {score}
            </div>
          ) : (
        ...
        <div className="flex flex-col items-center justify-center gap-1.5 px-2 py-3 shrink-0 min-w-[52px] md:min-w-[60px]">
          ...
          {isSelected ? (
            <div className={cn(
              ...
              isTiebreakerGame ? "bg-yellow-500/20 border-yellow-400" : "bg-purple-500/20 border-purple-400",
            )}>
              <Check className={cn("w-3 h-3", isTiebreakerGame ? "text-yellow-300" : "text-purple-300")} />
            </div>
          ) : (
    ...
    <div className="space-y-6">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="tb-passing">Combined passing yards — both QBs</Label>
              <Input
                id="tb-passing"
                type="number"
                min="0"
      ...
      <Dialog open={resultsWeek !== null} onOpenChange={(open) => { if (!open) setResultsWeek(null); }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {resultsWeek !== null && <NflConfidenceGrid poolId={poolId} initialWeek={resultsWeek} />}
        </DialogContent>
      </Dialog>
    </div>
  );
