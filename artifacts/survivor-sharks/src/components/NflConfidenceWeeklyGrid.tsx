...
function PickCell({ pick, game }: { pick: PlayerPick | undefined; game: GameSummary }) {
  ...
  return (
    <div className={cn(
      ...
      isWin ? "bg-green-500/10" : isLoss ? "bg-red-500/10" : "bg-cyan-500/5",
    )}>
      <img
      ...
      {pick.confidencePoints != null && (
        <span className={cn(
          ...
          isWin ? "text-green-300" : isLoss ? "text-red-300" : "text-cyan-300",
        )}>
    ...
    <div className="space-y-4">
      ...
      {displayGames.length > 0 && (
        <div className="flex items-center gap-4 text-[10px] text-muted-foreground px-1">
          ...
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded bg-cyan-500/10 border border-cyan-500/20" /> Pending
          </span>
